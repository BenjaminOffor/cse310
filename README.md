# JavaScript Quiz Program

## Description
A simple JavaScript quiz program that asks multiple-choice questions, calculates the score, and displays it using SweetAlert2. Demonstrates ES6 features, recursion, exception handling, and DOM manipulation.

## Features
- Browser-based quiz interface
- Uses ES6 array functions (`map`, `reduce`)
- Recursive function to iterate through questions
- Exception handling for errors
- SweetAlert2 for final score display
- Basic CSS styling

## Installation
1. Clone the repository: 
git clone https://github.com/BenjaminOffor/cse310.git

2. Open `index.html` in a browser.

## Usage
1. Answer the questions by clicking the buttons.
2. The score will display in a popup at the end.

## Video Demo
[Insert Zoom Video Link Here]